...
void DepositClient(string AccountNumber,double Amount, vector<stClient> &vClients) {
...
void TotalBanacess() {
    ...
    if (vClients.size() == 0)
    {
        cout << "\n\t NoClients available in the system\n";
    }
    else
    {
        for (stClient& C : vClients)
...
void GoBackToTransactionScreen() {
    cout << "Press Any key to go to transaction menu...";
    system("pause>0");
...
void PerformMainMenuOption(enMainMenuOptions SelectedOption) {
    ...
    case enMainMenuOptions::TranscationClients :
        system("cls");
        ShowTransactionsScreen();
...
void ShowMainMenu() {
    ...
    cout << "\t[6] transaction Client." << endl;
    cout << "\t[7] Exit." << endl;
    cout << "======================================================\n";
